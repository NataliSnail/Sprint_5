...
class TestAuthorization:
    ...
    def test_click_button_personal_account(self, login):
        '''авторизация через кнопку "Личный кабинет"'''
        # использую элемент "Оформить заказ" как подтверждение, что авторизация пройдена, т.к. без авторизации он недоступен
        assert WebDriverWait(login, 10).until(expected_conditions.visibility_of_element_located((Locators.CHECKOUT)))
    ...
    def test_click_forgot_password(self):
        ...
        WebDriverWait(driver, 5).until(expected_conditions.visibility_of_element_located((Locators.SAVE_BUTTON))).click()
